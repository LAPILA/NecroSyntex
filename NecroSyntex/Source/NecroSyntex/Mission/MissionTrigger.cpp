...
AMissionTrigger::AMissionTrigger()
{
	...
	MissionTriggerBox = CreateDefaultSubobject<UBoxComponent>(TEXT("SkillArea"));
...
void AMissionTrigger::BeginPlay()
{
	...
	if (MissionTriggerBox) {
		...
		MissionTriggerBox->OnComponentEndOverlap.AddDynamic(this, &AMissionTrigger::OnBoxTriggerOverlapBegin);
	}
	else {
...
void AMissionTrigger::OnBoxTriggerOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (!HasAuthority()) return;
